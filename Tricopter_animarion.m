...
%% Define design parameters
    ...
    drone(6) = surface(xcylinder,ycylinder+ala_y,H_m*zcylinder+H/2,'facecolor','r');
    drone(7) = surface(xcylinder,ycylinder-ala_y,H_m*zcylinder+H/2,'facecolor','r');
    alpha(drone(5:7),0.7);

    % design 3 propellers
    drone(8) = patch(xp-coda,yp,zp+(H_m+H/2),'c','LineWidth',0.5);  %tail
    drone(9) = patch(xp,yp+ala_y,zp+(H_m+H/2),'p','LineWidth',0.5);
    drone(10) = patch(xp,yp-ala_y,zp+(H_m+H/2),'p','LineWidth',0.5);
    ...
    xx = x(:,1);
    yy = x(:,2);
    zz = -x(:,3); % asse z positivo verso il basso
